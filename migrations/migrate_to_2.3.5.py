#!/usr/bin/env python3
"""
Datenbank-Migration: Version 2.3.5
Konsolidierte Migration für alle Versionen bis 2.3.5
...
- Passwort-vergessen-Funktion (password_reset_code, password_reset_code_expires)
...
def migrate_password_reset_fields():
    """Migration: Fügt Passwort-Reset-Felder zur users Tabelle hinzu."""
    print("\n" + "=" * 60)
    ...
    # Füge password_reset_code Spalte hinzu
    ...
    # Füge password_reset_code_expires Spalte hinzu
    if 'password_reset_code_expires' not in columns:
        print("[INFO] Füge password_reset_code_expires Spalte zu users Tabelle hinzu...")
        with db.engine.begin() as conn:
            conn.execute(text("ALTER TABLE users ADD COLUMN password_reset_code_expires DATETIME NULL"))
...
def migrate():
    ...
    with app.app_context():
        try:
            ...
            print("=" * 60)
            print()
            print("Die Datenbank wurde erfolgreich auf Version 2.3.5 aktualisiert.")
            return True
