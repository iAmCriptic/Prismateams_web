"""Per-folder IMAP sync pipeline: connect → fetch → upsert → flags."""
...
@dataclass
class FolderFetch:
    all_seq_numbers: list
    seq_to_uid: dict
    highest_uid: int | None
    emails_to_process: list
    sync_index: object



def connect_folder_for_sync(folder_name, mail_conn=None, mailbox=None):
    """Step 1 — IMAP connect + SELECT. Returns (ok, msg, state|None)."""
    ...
    try:
        if owns_connection:
            ...
            if not mail_conn:
                ...
                return False, f"IMAP-Verbindung fehlgeschlagen für Ordner '{folder_name}'", None
        else:
            ...
            if not ok:
                ...
                if "doesn't exist" in error_msg or "Mailbox doesn't exist" in error_msg or "NONEXISTENT" in error_msg:
                    ...
                    return True, f"Ordner '{folder_name}' existiert nicht auf dem Server, übersprungen", None
                logging.warning("IMAP folder selection failed for '%s': %s", folder_name, error_msg)
                return True, f"Ordner '{folder_name}' konnte nicht geöffnet werden, übersprungen: {error_msg}", None
    except Exception as conn_error:
        logging.error(f"Fehler beim Verbinden mit IMAP für Ordner '{folder_name}': {conn_error}")
        if owns_connection:
            _imap_logout(mail_conn)
        return False, f"IMAP-Verbindungsfehler: {str(conn_error)}", None
    ...
    try:
        ...
        if owns_connection:
            _imap_logout(mail_conn)
        return True, f"Ordner '{folder_name}' konnte nicht geöffnet werden, übersprungen: {str(e)}", None

    return True, None, (mail_conn, owns_connection, mailbox_id)



def fetch_folder_messages(mail_conn, folder_name, mailbox_id):
    """Step 2 — SEARCH + UID-Map + welche Mails neu sind. Returns (ok, msg, FolderFetch|None)."""
    ...
    if status != 'OK':
        ...
        return False, f"E-Mail-Suche in Ordner '{folder_name}' fehlgeschlagen: {messages[0].decode() if messages else 'Unbekannter Fehler'}", None
    ...
    if len(all_seq_numbers) == 0:
        ...
        return True, f"Ordner '{folder_name}': Keine E-Mails vorhanden", None
    ...
    from app.utils.email_sync_index import FolderSyncIndex
    ...
    return True, None, FolderFetch(
        all_seq_numbers=all_seq_numbers,
...
def upsert_folder_emails(mail_conn, folder_name, mailbox_id, fetched: FolderFetch, stats: dict) -> None:
    """Step 3 — RFC822 parsen und in der DB anlegen/aktualisieren/verschieben."""
    seq_to_uid = fetched.seq_to_uid
    emails_to_process = fetched.emails_to_process
    sync_index = fetched.sync_index
...
def apply_folder_flags(fetched: FolderFetch, stats: dict) -> None:
    """Step 4 — lokale Zeilen markieren, deren UID auf dem Server fehlt."""
    from app.utils.email_sync_index import mark_missing_server_uids

    highest_uid = fetched.highest_uid
    seq_to_uid = fetched.seq_to_uid
    all_seq_numbers = fetched.all_seq_numbers
    folder_name = fetched.sync_index.folder_name
    sync_index = fetched.sync_index
...
def sync_emails_from_folder(folder_name, mail_conn=None, mailbox=None):
    ...
    """
    ok, message, conn = connect_folder_for_sync(folder_name, mail_conn, mailbox)
    if conn is None:
        return ok, message
    mail_conn, owns_connection, mailbox_id = conn
    stats = _empty_stats()
    closed = False
    try:
        ok, message, fetched = fetch_folder_messages(mail_conn, folder_name, mailbox_id)
        if fetched is None:
            return ok, message
        apply_folder_flags(fetched, stats)
        upsert_folder_emails(mail_conn, folder_name, mailbox_id, fetched, stats)
        ...
        if owns_connection and not closed:
            _imap_logout(mail_conn)
